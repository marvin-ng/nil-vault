...
import { LayoutDashboard, Briefcase, ShieldCheck, DollarSign, Users, LogOut, Menu, X, LogIn, Zap } from "lucide-react";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground flex-col">
      {/* Demo mode banner */}
      {demoMode && (
        <div className="flex items-center justify-center gap-3 bg-primary/10 border-b border-primary/20 px-4 py-2 shrink-0">
          <Zap className="w-3.5 h-3.5 text-primary shrink-0" />
          <p className="text-xs text-primary font-medium">
            Demo mode — data is sample only, nothing is saved
          </p>
          <button
            onClick={signOut}
            className="text-xs text-muted-foreground hover:text-primary underline underline-offset-2 transition-colors ml-1"
          >
            Exit demo
          </button>
        </div>
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    {demoMode ? "Demo Account" : (profile.sport ?? (profile.role === "admin" ? "Admin" : ""))}
                  </p>
                </div>
                <Button
                  variant="ghost"
